...
const Register = () => {
  ...
  const onSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (password !== confirm) {
      toast({ title: "Ошибка", description: "Пароли не совпадают" });
      return;
    }
    toast({ title: "Аккаунт создан", description: "Теперь вы можете войти" });
  };
      ...
      <div className="container relative z-10 max-w-md w-full">
        ...
        <Card className="p-6 sm:p-8 bg-gradient-to-br from-card to-secondary/30 border-primary/20 shadow-glow transform transition-transform duration-300 hover:-translate-y-1">
          ...
          <CardContent>
            ...
            <Button type="submit" className="w-full transform transition-transform duration-200 hover:-translate-y-0.5">Создать аккаунт</Button>
            </form>
          </CardContent>
          <CardFooter className="justify-between">
            <div className="text-sm text-muted-foreground">Уже есть аккаунт?</div>
            <Link to="/login" className="text-primary hover:text-primary/90 transform transition-transform duration-200 hover:-translate-y-0.5">Войти</Link>
          </CardFooter>
